 ...
 * Orchestrates: PDF.js → Image Enhancement → Multi-pass OCR → Name Engine → Data Extractor → Black-bar Redaction
 */
const EuaaPdfEnhanced = (function () {
  ...
  function blackBar(ctx, x, y, w, h, pad) {
    ctx.fillStyle = '#000';
    ctx.fillRect(Math.floor(x - pad), Math.floor(y - pad), Math.ceil(w + pad * 2), Math.ceil(h + pad * 2));
  }
  ...
  function findPdfOffset(bytes) {
    var lim = Math.min(bytes.length, 4096);
    for (var i = 0; i < lim - 4; i++) {
  ...
  /**
   * process(file, active, useOcr, padding, manTerms, onStatus)
   * Returns Promise<result>
   * result: { sourceName, error, barCount, canvases, pageData[], downloads, summary }
   */
  function process(file, active, useOcr, padding, manTerms, onStatus) {
    ...
    return file.arrayBuffer().then(function (buffer) {
      ...
      var totalBars = 0;
      var canvases = [];
      var pageData = [];  // per-page OCR + extraction results
      var allPageNames = [];
      var allPageFields = [];
      ...
      function nextPage() {
        if (pageIndex >= pdf.numPages) {
          // All pages done — run cross-page consistency checks
          status('Running consistency checks…', 97);

          var crossRef = null;
          if (window.EuaaNameEngine) {
            crossRef = EuaaNameEngine.crossReferenceNames(allPageNames);
          }
          var consistencyFlags = [];
          if (window.EuaaDataExtractor) {
            consistencyFlags = EuaaDataExtractor.checkConsistency(allPageFields, allPageNames);
            consistencyFlags = EuaaDataExtractor.generateSuggestions(consistencyFlags);
          }
          ...
          var allFields = allPageFields.reduce(function (acc, pf) {
            Object.keys(pf).forEach(function (k) {
              if (!acc[k]) acc[k] = [];
              acc[k] = acc[k].concat(pf[k]);
            });
            return acc;
          }, {});
          ...
          return {
            ...
            canvases:         canvases,
            pageData:         pageData,
            downloads:        [{ filename: outName, blob: new Blob([outBuf], { type: 'application/pdf' }) }],
            summary: {
              pages:           pdf.numPages,
              totalBars:       totalBars,
              names:           allNames,
        ...
        return pdf.getPage(pageNum).then(function (pdfPage) {
          ...
          return pdfPage.render({ canvasContext: ctx, viewport: vp }).promise.then(function () {
            ...
            var pageWords = []; // words from PDF.js text layer

            // ── PDF.js text layer redaction ──
            content.items.forEach(function (item) {
              ...
              var ents = (active.size && window.EuaaAnonymizer)
                ? EuaaAnonymizer.detectEntities(str, 'demo-safe', active)
                : [];
              var needsRedact = ents.length > 0;
              if (!needsRedact && manRx) { manRx.lastIndex = 0; needsRedact = manRx.test(str); manRx.lastIndex = 0; }

              // Build word objects for name engine
              ...
              pageWords.push({
                ...
                bbox: {
                  x0: t[4] * SCALE,
                  y0: (t[5] - (item.height || 10)) * SCALE,
                  x1: (t[4] + (item.width || str.length * 5)) * SCALE,
                  y1: t[5] * SCALE,
                }
              });
              ...
              if (needsRedact) {
                var cx = t[4] * SCALE;
                var cy = t[5] * SCALE;
                var cw = Math.max((item.width || str.length * 6) * SCALE, 20);
                var ch = Math.max((item.height || Math.abs(t[3]) || 10) * SCALE, 10);
                blackBar(ctx, cx, cy - ch, cw, ch, pad);
                totalBars++;
              }
            });

            // ── OCR fallback for image pages (or always if enhanced OCR enabled) ──
            ...
            if (useOcr && window.EuaaOcrEngine && (!pageHasText || active.has('ENHANCED_OCR'))) {
              ...
              ocrPromise = EuaaOcrEngine.ocrCanvas(canvas, {}, function (p, msg) {
                status(msg || ('OCR ' + pageNum + '…'), pct + p * 8);
              }).then(function (ocrResult) {
                // Redact OCR-found words
                (ocrResult.words || []).forEach(function (w) {
                  ...
                  var ents = (active.size && window.EuaaAnonymizer)
                    ? EuaaAnonymizer.detectEntities(str, 'demo-safe', active)
                    : [];
                  var needs = ents.length > 0;
                  if (!needs && manRx) { manRx.lastIndex = 0; needs = manRx.test(str); manRx.lastIndex = 0; }
                  if (!needs) return;
                  var b = w.bbox || {};
                  blackBar(ctx, b.x0 || 0, b.y0 || 0, (b.x1 || 0) - (b.x0 || 0), (b.y1 || 0) - (b.y0 || 0), pad);
                  totalBars++;
                });
                // Merge word lists
                return {
                  text: ocrResult.text || pageText,
                  words: pageWords.concat(ocrResult.words || []),
                  confidence: ocrResult.confidence,
                  detectedScript: ocrResult.detectedScript,
                  passCount: ocrResult.passCount,
                };
              }).catch(function () {
                return { text: pageText, words: pageWords, confidence: 85, detectedScript: 'latin' };
              });
            }
            ...
            // ── Name extraction ──
            var nameSpans = [];
            if (window.EuaaNameEngine && ocrResult.words && ocrResult.words.length) {
              nameSpans = EuaaNameEngine.extractNameSpans(ocrResult.words);
            }
            allPageNames.push(nameSpans);

            // ── Field extraction ──
            var fields = {};
            if (window.EuaaDataExtractor && ocrResult.text) {
              fields = EuaaDataExtractor.extractAll(ocrResult.text, ocrResult.words);
            }
            allPageFields.push(fields);

            // ── Document type (first page only) ──
            var docType = null;
            if (pi === 0 && window.EuaaDataExtractor) {
              docType = EuaaDataExtractor.classifyDocument(ocrResult.text);
            }
